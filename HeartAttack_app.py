...
def prediction(PhysicalHealthDays, GeneralHealth, RemovedTeeth, HadAngina, HadStroke, HadCOPD, HadKidneyDisease, HadArthritis, HadDiabetes, DeafOrHardOfHearing,
    ...
    if prediction < 0.5:
    	pred = st.success(f'Great news! Based on the information provided you have a probability of {prediction:.2f}% to have a heart attack. Keep maintaining a healthy lifestyle.')
    else:
    	pred = st.error(f'Important: The prediction indicates a potential risk of a heart attack with a probability of {prediction:.2f}%. It is crucial to consult a healthcare professional.')
...
def main():       
    ...
    **Take Charge of Your Heart Health Today!**
    ...
    PhysicalHealthDays = st.number_input('How many days during the past 30 days was your physical health not good? (last 30 days)', 0, 30, 0)
    ...
    if st.button("Predict"): 
    	result = prediction(PhysicalHealthDays, GeneralHealth, RemovedTeeth, HadAngina, HadStroke, HadCOPD, HadKidneyDisease, HadArthritis, HadDiabetes, DeafOrHardOfHearing,
                            DifficultyWalking, SmokerStatus, ChestScan, AgeCategory, PneumoVaxEver)
    	st.write(result)
